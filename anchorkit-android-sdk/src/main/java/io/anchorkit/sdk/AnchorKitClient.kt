...
class AnchorKitClient(
    ...
    suspend fun fetchChallenge(): AttestationChallenge = withContext(Dispatchers.IO) {
        val connection = openConnection("$baseUrl/api/v1/attestation-challenge", "GET")
        try {
    ...
    suspend fun submitHash(
        ...
        val connection = openConnection("$baseUrl/api/v1/submit-hash", "POST")
        try {
    ...
    suspend fun verifyHash(hash: String): VerificationResult = withContext(Dispatchers.IO) {
        val connection = openConnection("$baseUrl/api/v1/verify-hash/${hash.lowercase()}", "GET")
        try {
    ...
    suspend fun subscribeToNotifications(email: String): String = withContext(Dispatchers.IO) {
        ...
        val connection = openConnection("$baseUrl/api/v1/notifications/subscribe", "POST")
        try {
    ...
    suspend fun unsubscribeFromNotifications(email: String): Unit = withContext(Dispatchers.IO) {
        ...
        val connection = openConnection("$baseUrl/api/v1/notifications/unsubscribe-api", "POST")
        try {
    ...
    suspend fun downloadProof(hash: String): PortableProof = withContext(Dispatchers.IO) {
        val connection = openConnection("$baseUrl/api/v1/proof/${hash.lowercase()}", "GET")
        try {
